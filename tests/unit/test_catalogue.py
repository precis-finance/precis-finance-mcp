...
class TestDimensionLoading:
    ...
    def test_cost_centre_is_leaf(self):
        ...
        assert dim.source_table == "live.dim_cost_centre"
    ...
    def test_transitive_closure_computed(self):
        ...
        assert tr.source_table == "live.dim_cost_centre"
...
class TestMultiDimensionCube:
    def test_two_cube_dims_same_source(self, tmp_path):
        write_yml(tmp_path, "dimensions.yml", """
            dimensions:
              cost_centre:
                label: Cost Centre
                source:
                  table: gl.dim_cost_centre
                  key_column: cost_centre_id
        """)
        write_yml(tmp_path, "domain.yml", """
            domain: interco
            source_view: semantic.v_interco
            dimensions:
              - key: base_cost_centre
                label: Cost Centre
                source: cost_centre
              - key: counterparty_cost_centre
                label: Counterparty
                source: cost_centre
                prefix: Cpty
            metrics:
        ...
        """)
        cat = load_catalogue(str(tmp_path))
        domain = cat.domains["interco"]
        assert len(domain.dimensions) == 2
        base = domain.dimensions[0]
        cpty = domain.dimensions[1]
        assert base.key == "base_cost_centre"
        assert base.prefix == ""
        assert base.source == "cost_centre"
        assert cpty.key == "counterparty_cost_centre"
        assert cpty.prefix == "Cpty"
        assert cpty.source == "cost_centre"
        # Both reference the same master dimension
        assert base.source == cpty.source
