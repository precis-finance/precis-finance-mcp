...
indexes. The table moved into the squashed open init during the migration
history reorg; before that it lived in the per-step `025_load_history.sql`.
Architectural framing in `docs/architecture/03-integration.md` §"Key invariants" #4.
They run without any database.
