...
def _build_ragged_catalogue(root: Path) -> object:
    """A minimal catalogue with one `type: generated` ragged hierarchy."""
    cat_dir = root / "catalogue"
    cat_dir.mkdir(parents=True)
    (cat_dir / "dimensions.yml").write_text(textwrap.dedent("""
        dimensions:
          branch:
            label: Branch
            source:
          ...
          org:
            ...
            source:
              type: generated
    """))
    return load_catalogue(str(cat_dir))


def test_apply_all_materialises_ragged_views_between_dims_and_views(tmp_path: Path):
    """With a catalogue, the generated ragged views land after the file dims
    (their leaf source may be a file dim) and before the file views (which may
    reference them)."""
    semantic = tmp_path / "semantic"
    _build_semantic_tree(semantic)
    catalogue = _build_ragged_catalogue(tmp_path)
    ...
    passthrough_cmd = next(c for c in ch.commands if "semantic.dim_branch AS" in c)
    assert "FROM live.dim_branch" in passthrough_cmd
    # The generated rollup references the normalised semantic source (no Jinja).
    rollup_cmd = next(c for c in ch.commands if "semantic.dim_branch_org_rollup" in c)
    assert "FROM semantic.dim_branch" in rollup_cmd
    assert "{{" not in rollup_cmd


def test_apply_all_without_catalogue_skips_ragged_views(tmp_path: Path):
    """No catalogue → only the file-based views are applied (back-compat)."""
    _build_semantic_tree(tmp_path)
    ch = _StubCH()
    report = apply_all(tmp_path, ch)
    assert report.views_applied == ["dim_account", "v_gl"]
