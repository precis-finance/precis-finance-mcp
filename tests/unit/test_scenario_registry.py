...
def test_real_metadata_rows_are_shared_route_shape(registry: ScenarioRegistry):
    ...
    assert actuals["scenario"] == "actuals"
...
def test_reporting_vocabulary_sections(registry: ScenarioRegistry):
    ...
    assert {entry["scenario"] for entry in vocabulary["real"]} >= {"actuals", "budget"}
    assert {entry["scenario"] for entry in vocabulary["shifted"]} >= {
        "actuals_py",
        "forecast_q2_pp",
    }
    assert {entry["scenario"] for entry in vocabulary["comparisons"]} >= {
        "actuals_vs_budget",
        "forecast_q2_vs_forecast_q1_pct",
    }
    assert {"scenario": "prior_year", "resolves_to": "actuals_py"} in (
...
def test_vocabulary_includes_self_yoy_comparisons(registry: ScenarioRegistry):
    ...
    """
    vocabulary = registry.to_reporting_vocabulary()
    keys = {entry["scenario"] for entry in vocabulary["comparisons"]}
    ...
    actuals_yoy = next(
        e for e in vocabulary["comparisons"] if e["scenario"] == "actuals_vs_actuals_py"
    )
    assert actuals_yoy["scenario_ids"] == ["ACTUALS"]
    assert actuals_yoy["display_format"] == ""

    actuals_yoy_pct = next(
        e for e in vocabulary["comparisons"] if e["scenario"] == "actuals_vs_actuals_py_pct"
    )
