...
"""End-to-end OAuth2 proof for the open-core MCP connector.
...
This is the formal artifact for the split's definition of done (spec §12):
"the MCP-external connector completes the OAuth2 flow and a read end-to-end
against the open package alone."

Class 3 per `tests/CLAUDE.md` — marked `slow`. Skipped by default; auto-skips
...
realm client (decided 2026-06-04). This deliberately does not drive the
...
this proves is the integration-risk half the split actually owns:
  ...
  - a read tool returns ``structuredContent`` — over the network, against the
    open package with no commercial package in the loop.
