...
def test_readyz_ready_when_deps_reachable(monkeypatch):
    _default_mode(monkeypatch)
    monkeypatch.setattr("precis_mcp.db.query_platform", lambda *a, **k: [{"1": 1}])
    monkeypatch.setattr("precis_mcp.db.query_clickhouse", lambda *a, **k: [{"1": 1}])
    from precis_mcp.app_open import app

    with TestClient(app) as c:
        r = c.get("/readyz")
    assert r.status_code == 200
    assert r.json() == {"status": "ready", "checks": {"postgres": "ok", "clickhouse": "ok"}}


def test_readyz_503_when_a_dependency_is_down(monkeypatch):
    _default_mode(monkeypatch)
    monkeypatch.setattr("precis_mcp.db.query_platform", lambda *a, **k: [{"1": 1}])
    ...
    monkeypatch.setattr("precis_mcp.db.query_clickhouse", _boom)
    from precis_mcp.app_open import app
    ...
    assert body["checks"]["clickhouse"].startswith("error")
