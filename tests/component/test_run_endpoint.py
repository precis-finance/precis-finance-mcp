...
(G1 — same key + different body → 409 Conflict), and the orchestrator
failure path.
...
def _verifier(*binding_ids: str, roles: tuple[str, ...] = ("admin",)):
    ...
    `roles` defaults to `("admin",)` so happy-path tests pass the §11.3 role
...
# ---------------------------------------------------------------------------
# Idempotency (G1: body-hash check)
# ---------------------------------------------------------------------------
...
def test_run_endpoint_repeat_key_different_body_returns_409(tmp_path: Path):
    """G1: reusing an idempotency_key with a different body must return 409
...
# ---------------------------------------------------------------------------
# override_lock + notes (Phase 3 — admin re-trigger audit fields)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Role gate (spec §11.3): re-trigger requires admin or plan_manager;
