...
def test_all_expected_tools_registered(tools):
    ...
    # tools — they moved to the commercial plan_read_tools module.
...
def _set_auth(user_id: str, allowed: list[str] | None, is_admin: bool = False):
    ...
    ``list_scenarios`` tool (post-H-09b) reads
...
def test_list_scenarios_includes_semantic_registry(tools, fake_ch):
    ...
    assert result["registry"]["real"][0]["key"] == "actuals"
    assert {s["key"] for s in result["registry"]["shifted"]} >= {
        "actuals_py",
        "budget_pp",
    }
    comparison_keys = {s["key"] for s in result["registry"]["comparisons"]}
...
# ---------------------------------------------------------------------------
# list_scenarios — per-user filtering (H-09b regression)
# ---------------------------------------------------------------------------


class TestListScenariosFilter:
    """H-09b: ``list_scenarios`` must filter ``real``, ``shifted``,
...
def test_safe_execute_catches_generic_exception(tools):
    ...
    # M-02b: driver exception text (which can embed SQL) must not reach the
