...
The import-linter proves there are no open→commercial *imports*. This proves the
open dispatch core + open tool set actually *assemble and serve a query* with no
commercial tools registered — i.e. without ``commercial_tools.install()``. It
catches runtime coupling the linter cannot see: an open tool that reads state,
config, or a catalogue entry only the commercial half provides (ADR-0008).

The session conftest installs the commercial loader (mirroring agui startup), so
...
# A sample of commercial tools that must be ABSENT from the open assembly.
# run_validation_check is commercial — plan-entry validation only exists under
# the write path (ADR-0009). web_fetch is commercial — it depends on the
# Anthropic SDK + an API key, outside the open core's zero-LLM read surface.
COMMERCIAL_ABSENT = {
...
def open_only(monkeypatch):
    """Assemble as an open-only deployment: strip the commercial tool loader +
    ...
    (including the cache) after the test, so the commercial set is back for the
    next test."""
...
def test_open_dispatch_assembles_exactly_the_open_tool_set(open_only, catalogue):
    """With no commercial loader registered, build_descriptors yields exactly the
    ...
    # The open core is read-only: no write-class tools assemble.
    assert all(
...
@pytest.mark.skipif(
    ...
    reason="monorepo-only: drives the open /mcp transport through the "
    "commercial agui app (the `client` fixture) to prove the split in situ",
)
def test_open_core_serves_a_read_query_over_mcp(open_only, client, ch_client):
    ...
    executes a read tool (list_kpis) with no commercial tools registered."""
    ...
    assert "eval_chart_transform" not in names   # a commercial mcp_read tool is absent
...
def test_app_open_entrypoint_boots_standalone(open_only, ch_client):
    ...
    serves the open surface, mounting none of the commercial routers. This is
    the independence proof of spec §12: the open package serves reads over its
    own app, with no commercial application present."""
    from fastapi.testclient import TestClient

    from precis_mcp.app_open import app

    # The open app mounts only the open surface — no commercial routers.
...
def test_open_instructions_exclude_commercial_surface(monkeypatch, catalogue):
    ...
    no charts (`eval_chart_transform`), no Excel (`_excel`). The commercial
    ...
    The conftest installs the commercial tools (and thus the template override)
