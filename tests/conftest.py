...
This file is the root of the test-runtime contract. It carries:
...
- The open test boundary guard that fails the run when a file listed
  in `tests/open_tests.txt` imports the `precis` package.
- The canonical shared fixtures (`db`, `ch_client`, `auth_token`,
  `auth_headers`, `redis`, `client`).

The shared fixtures define what each fixture promises. Marker registration
lives in `pyproject.toml` `[tool.pytest.ini_options]`.
"""
...
# Set CHECKPOINT_DIR before any module import so the app's module-level init
...
# ---------------------------------------------------------------------------
# Open-core test boundary guard — see tests/open_tests.txt.
#
# The manifest lists every test that exercises ONLY the open `precis_mcp`
# package and is therefore extracted to the open repo. Those files must not
# import the `precis` package — otherwise extraction breaks. The
# package-level import-linter contract checks `precis_mcp` vs `precis` but not
# `tests/`, so a new open test could import `precis` unnoticed without this guard.
# ---------------------------------------------------------------------------
...
def _commercial_imports(imports: set[str]) -> set[str]:
    """Subset of `imports` that reaches the `precis` package.
...
def pytest_collection_modifyitems(config, items):
    ...
    file listed in `tests/open_tests.txt` imports the `precis`
    package.
    ...
    if violations:
        lines = [
            "Unit tests with forbidden real-service imports:",
        ]
    ...
    if open_violations:
        lines = [
            "Open tests importing the `precis` package (see "
            "tests/open_tests.txt):",
        ]
        for p, mods in _dedupe(open_violations):
            lines.append(f"  {p}: {', '.join(mods)}")
        lines.append(
            "Fix: drop the `precis` import (use `precis_mcp` equivalents), or "
            "remove the file from tests/open_tests.txt if it genuinely "
            "belongs to the Précis platform."
        )
        error_blocks.append("\n".join(lines))
...
# continue to win (closer scope). These conftest fixtures activate as each
# file's inline fixture is removed.
# ---------------------------------------------------------------------------
...
# Register the Précis tool set into the open dispatch core so
