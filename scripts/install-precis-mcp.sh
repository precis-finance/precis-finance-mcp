...
#
# The lean counterpart to scripts/install.sh (the Précis bootstrap).
...
# No gVisor/sandbox, no Précis nginx vhost, no Précis secrets — the
...
#
# --nginx is opt-in (not part of the default run), and most installs don't
# need it: the bundle's default ingress is its own Caddy auto-HTTPS proxy
# (the bundled-proxy compose profile — set PRECIS_DOMAIN in deploy/.env and
# you're done). Use --nginx only when you'd rather run host nginx + certbot
# (e.g. nginx already serves other vhosts on this box — our own operated
# topology). It needs a real domain whose DNS already points at this host, so
# certbot can obtain a cert. It installs the deploy/nginx/<domain>.conf vhost
# fronting the bundle's localhost ports (Keycloak :8080 under /auth, app_open
# :8769 for the rest); drop bundled-proxy from COMPOSE_PROFILES so the two
# proxies don't fight over ports 80/443.
#
...
if $RUN_FIREWALL; then
    ...
    ssh "$SERVER" "
        ...
        # 22 first so enabling never drops the SSH session.
        ufw allow 22/tcp
        ufw allow 80/tcp
        ufw allow 443/tcp
        ufw --force enable
...
# Host nginx fronts the bundle's localhost ports (the Précis topology):
