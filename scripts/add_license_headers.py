...
Every file in the open `precis-mcp` distribution carries a short licence
header. This is the scripted application.
...
target set). The header is inserted after any shebang and/or
...
recursively for `*.py`). Pass only the OPEN tree — never the Précis platform
