...
--
-- This is the open-core half of the platform database: identity-as-credential,
-- the profile/permission model, the security + inspection audit logs, the
...
-- commercial -> open, never the reverse; ADR-0005).
--
-- ORIGIN. This file is the squash of the open-owned tables from the original
-- flat history (platform/001..030), captured at their FINAL shape after every
-- ALTER. The per-step history is preserved in git; it was collapsed here because
-- all live deployments were already at HEAD, which is the precondition that
-- makes a squash safe. See docs/decisions/0005-...md and the migration README.
--
...
-- in the commercial user_profile_ext table, not here (ADR-0005).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS users (
    id          TEXT PRIMARY KEY,
    entity_id   TEXT NOT NULL DEFAULT 'ENT-001',
...
-- profile, so access grants are never silently lost.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS user_profile_assignments (
    ...
    source     TEXT NOT NULL DEFAULT 'admin_ui'
               CHECK (source IN ('seed', 'admin_ui', 'api', 'sso_sync')),
...
-- ---------------------------------------------------------------------------
-- security_audit_log — append-only record of security-relevant events.
-- ---------------------------------------------------------------------------
...
-- months ('YYYY-MM', 'YYYY-13', 'YYYY-MM-ADJ' all valid).
-- ---------------------------------------------------------------------------
