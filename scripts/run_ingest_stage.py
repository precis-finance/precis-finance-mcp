...
Postgres + Ibis backends) — identical to what cron / push /
...
# Make `precis_mcp` importable when run from the repo root without an
# installed package.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

import precis_mcp.secrets  # noqa: E402,F401 — resolve *_FILE before any getenv on a source credential
...
def main() -> int:
    ...
    if ctx is None:
        print(
            ...
            "/ Postgres connectivity and that "
