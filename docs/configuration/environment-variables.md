...
| `EXCEL_ADDIN_SCOPE` | `openid` | Exact scope the add-in requests (advertised in the protected-resource metadata as `precis_excel_scopes`). Keep the default for an IdP that binds the audience via the resource indicator (Keycloak/Auth0/Ping). For an IdP that binds it via a scope (Entra/Okta), set the full request scope, e.g. `openid profile email offline_access api://<app-id>/access_as_user` (include `offline_access` so the add-in gets a refresh token, and `profile email` so the task pane can show the signed-in user's name/email instead of an opaque subject id). |
