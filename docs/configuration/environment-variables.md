...
|---|---|---|
| `COMPOSE_PROFILES` | `bundled-clickhouse,bundled-keycloak` | Which optional services the multi-user stack runs. Drop `bundled-clickhouse` to use your own ClickHouse; drop `bundled-keycloak` for a direct external IdP; append `backup` to run the [backup scheduler sidecar](../operations/backups.md); append `ingestion` and/or `ingestion-watch` for the [ingestion daemons](ingestion.md#scheduling). |
...
A `Source` of `kind: postgres` names a `secret_ref`; credentials are read from
`<SECRET_REF>_*` (uppercased). Example: `secret_ref: customer_pg` →
`CUSTOMER_PG_HOST`, `CUSTOMER_PG_DATABASE`, `CUSTOMER_PG_USER`,
`CUSTOMER_PG_PASSWORD` (**secret**), `CUSTOMER_PG_PORT` (default `5432`),
`CUSTOMER_PG_SSLMODE`, `CUSTOMER_PG_SSLROOTCERT`. See
