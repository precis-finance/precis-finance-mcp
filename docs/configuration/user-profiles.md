...
## Reviewing access and the audit log

Two read-only commands answer the questions an operator or auditor asks before
and after a grant:

```bash
# What can this user actually read? (effective scenarios, roles, scopes)
python -m precis_mcp.admin_cli show-access --user bob

# Who can read a given scenario?
python -m precis_mcp.admin_cli show-access --scenario budget-2026

# Who was granted what, and when? (filterable, JSON for export)
python -m precis_mcp.admin_cli audit --event profile_assigned
python -m precis_mcp.admin_cli audit --target bob --limit 50
```

`show-access` resolves the *effective* access a profile grants — not just the
assigned profile id — so it needs ClickHouse reachable to read the live
scenario list. Every privilege-grant operation (user create, admin
grant/revoke, disable, password reset, profile assign/revoke) writes a row to
`security_audit_log`, which `audit` reads back; filter by `--actor`,
`--target`, `--event`, `--since`, and `--limit`. Rows carry a `trace_id` when
the action ran on the agent/MCP path, so an entry joins to its trace.

## What to know

- **The admin flag is not data access.** `is_admin` does not bypass the
  scenario check — an admin who should also query data needs a profile
  like anyone else. In this package the flag has almost no runtime effect:
  no admin-class tool is published over `/mcp`, and user/profile management
  runs through the admin CLI, which is trust-bounded by host access, not by
  the flag. Its one visible effect is that metadata listings
  (`list_scenarios`, `list_variants`) show every scenario regardless of
  profile. The flag chiefly exists for products built on this package,
  whose administrative surfaces it gates.
