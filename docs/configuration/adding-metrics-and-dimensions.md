...
Postgres is currently the only supported federated source kind; the dispatch
in `precis_mcp/engine/ibis_registry.py::_connect_for_source` is the extension
point for others. See
...
catalogue load raises `CatalogueError`. Rewrite the filter as a structured
