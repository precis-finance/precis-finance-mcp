...
Before writing the DDL, settle the **grain** — what to land versus leave
federated, and the partition/scenario rules that follow from it:
[Designing the staging/live grain](../configuration/catalogue-and-semantic.md#designing-the-staginglive-grain).
...
Credentials resolve from env vars named by `secret_ref` uppercased — for a
Postgres source `<REF>_HOST`, `_USER`, `_PASSWORD`, `_DATABASE`, optional
`_PORT` / `_SCHEMA`; other kinds (Snowflake, BigQuery, Databricks, MSSQL) read
their own suffixes — with `*_FILE` indirection supported — see
...
- **A data-quality check failed** (`failed_checks`) — the data is wrong and
  **nothing landed** (no swap). Inspect `control_total_result` via
  `get_load_status` for the failing check and its detail (failing count or
  reconcile gap); fix the source data — or the check rule, if that is what's
  wrong — then re-run. A *warning*-severity check does not block: the load
  lands (`status='success'`) with the detail recorded for review.
- **A backfill partly failed** — re-running multiple periods is per-period
  atomic. Each `(binding, period)` swaps independently, so the periods that
  succeeded are committed and only the failed ones need re-running; there is
  nothing to roll back. Query `load_history` for the failed periods and
  re-trigger just those.
