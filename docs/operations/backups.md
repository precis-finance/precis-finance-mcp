...
4. **Render and check:** `precis-mcp-admin backup init`. Writes the
   ...
   (mode 0600 — for S3 it embeds the resolved writer credential, which is why
   it lives in the secrets area and is regenerated, never hand-edited),
   probes the destination for writability, and checks whether ClickHouse
   sees the `precis_backups` disk — on first run that check warns, expected.
...
5. **Mount and restart ClickHouse:** set
   ...
   `backup init` — the disk check now passes.
