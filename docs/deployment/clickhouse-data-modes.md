...
|---|---|---|---|
| `bundle-sample` | bundled (this stack runs it) | synthetic demo data | you want to try Précis-MCP with data to explore |
...
the synthetic generator for demo data). So the un-named combination — your own
cluster *with* demo data, e.g. a populated demo on infrastructure you already
run — is just "empty `COMPOSE_PROFILES` + `CHHOST=<yours>`, then run the
