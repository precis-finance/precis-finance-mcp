...
- **Ingress**: the app is proxy-agnostic; TLS terminates at your ingress.
...
- **Anonymous client registration on the bundled Keycloak.** Open by design
  ...
  records: rate-limit it, and close it entirely if you don't use the public
  connectors —
  [hardening detail](keycloak-brokering.md#the-dcr-and-admin-surfaces).
- **The Keycloak admin console.** The reference ingress proxies all of
  `/auth/` — including `/auth/admin/` — publicly, guarded only by the
  bootstrap admin password. Block or IP-allowlist it; reach it over a
  tunnel.
