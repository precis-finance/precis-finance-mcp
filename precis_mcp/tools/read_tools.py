...
Both support an ``out`` parameter controlling output destination:
  ...
  - 'report': persist result as a financial_table block on a saved report
    (via report_sink), return confirmation only
"""
...
def register_read_tools(mcp: FastMCP, ref: "CatalogueRef"):
    ...
    def _build_caption(
        ...
        *,
        ...
        so both the render path (agui.py) and the Excel export can use it
    ...
    def list_scenarios() -> dict:
        ...
        visible_real_keys = {row["key"] for row in real_rows}
    ...
    def run_statement(
        ...
        Args:
            ...
            scenarios: List of scenario dicts, each with 'scenario' (registry
                key) and optional 'alias' (display label). Use the scenario
                keys from **Available Scenarios** in the data model description
                (real, shifted, and generated comparison types). If a key is not recognised,
                call ``list_scenarios`` to discover valid keys.
        ...
        # In the agent flow, these are always set by apply_report_context().
        ...
        blocks = []
        for sc in scenarios:
            scenario_key = _resolve_scenario(sc.get("scenario", "actuals"))
        ...
        # Chart-result cache (data_ref) — commercial only. The open read path
        # writes no data_ref and touches no Redis; the cache is a registered
        # commercial chart-enablement step (read_tool_hooks).
        if isinstance(result, dict) and "error" not in result:
