...
# model calls it). Not in TOOL_CATALOGUE — no SPA impact.
    ...
    try:
        ...
        db.execute_platform(
            ...
            INSERT INTO security_audit_log
                (event_type, actor_id, scenario_id, details)
            VALUES (%s, %s, %s, %s::jsonb)
            """,
            (event_type, actor_id, scenario_id, json.dumps(payload)),
        )
    except Exception:
        logger.exception(
...
def _advertised_tools(permissions: UserPermissions) -> list[dict]:
    ...
    for name, desc in _descriptors.items():
        ...
        # report_id/position belong to the SPA-only report path).
        schema = _strip_dead_params(schema)
        # MCP-accurate description (overrides the SPA docstring, which documents
        ...
        # available (e.g. the commercial `_excel` variant is dropped when file
...
# out='report' params and report is rejected on MCP (the Précis report builder
# is SPA-only). Keeping them would advertise dead arguments the model might try.
...
def _apply_presentation_defaults(tool_name: str, cleaned: dict, params) -> None:
    ...
    and one decimal. The SPA seeds these from a per-conversation report context;
    ...
    passing `scale` / `decimals` explicitly. Not applied on the SPA — this lives
    only in the MCP dispatch path."""
...
async def _handle_initialize(
    ...
    # model's context (the MCP analogue of the SPA skill layer). Best-effort —
...
# ---------------------------------------------------------------------------
# Render-block seam (ADR-0004) — open builders native, commercial registered.
# ---------------------------------------------------------------------------
# The open MCP transport renders its own two block types directly from pure
# builders (no `streaming`/`emit_blocks` dependency). The commercial product
...
# open core. A builder takes the JSON-normalised tool result + the tool args +
...
def register_mcp_render_builder(tool_name: str, builder) -> None:
    ...
    The open core registers the finance-table and inspection-grid builders
    natively; the commercial product calls this at startup to add its premium
...
def _derive_render_block(
    ...
    commercial chart builder is registered at startup. Returns None when no
    ...
    what the SPA emitters see (the `json.dumps(..., default=str)` round-trip)."""
