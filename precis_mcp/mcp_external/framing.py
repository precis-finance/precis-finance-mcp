...
# bundles ship in `ui/mcp-widgets/dist/`.  Keyed by the *render-variant* MCP name
...
# *definition* — there is no per-call render toggle like Précis's `out=` — so
...
class McpOutputMode:
    """A Précis extension adding a split-tool variant + its result framer.
    ...
    the Précis platform at startup via `register_mcp_output_mode`. An open
...
def register_mcp_output_mode(mode: McpOutputMode) -> None:
    """Register a Précis output mode (variant + framer). Keyed by `out`."""
...
def mcp_tool_variants(tool_name: str) -> list[tuple[str, str]]:
    ...
    output mode (Précis; e.g. `_excel`). A widget-linked tool that isn't
...
# MCP-accurate tool descriptions. The Précis docstrings document `out=`, the report
...
# Précis description overrides — a Précis tool's MCP-accurate
...
def register_mcp_tool_description(tool_name: str, description: str) -> None:
    """Register a Précis tool's MCP description. Idempotent; last wins."""
...
def mcp_tool_description(tool_name: str) -> str | None:
    ...
    docstring's first line. A Précis override (registered at startup) wins
    over the open table."""
    ...
    *,
    ...
    if out == "report":
        raise FramingError(
            ...
            "report builder is in the Précis app.  Use out='render' or out='excel'."
        )
