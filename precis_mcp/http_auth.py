...
Open-core module: request-level authorization dependencies shared by every admin
surface — the commercial admin REST routes and the open ingestion admin routes.
Kept out of the route modules so the open ingestion router can gate on admin
access without importing a commercial ``routes.*`` module.
