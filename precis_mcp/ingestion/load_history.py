...
# ---------------------------------------------------------------------------
# Module-level read helpers — used by the agent read tools, independent of
...
def query_load_history(
    *,
    ...
    ordered most-recent-first. Used by `list_load_history` agent tool."""
    clauses: list[str] = []
    params: list[Any] = []
...
def get_load_history_row(load_id: str) -> Optional[dict[str, Any]]:
    ...
    Used by `get_load_status` agent tool."""
