...
class BindingScopedToken:
    ...
    `override_lock` path (see `docs/architecture/06-identity-and-access.md`
    for the role model). The upload endpoint
    ...
    async def upload(
        ...
        if x_idempotency_key:
            if cache.seen(x_idempotency_key):
                ...
                # Per spec §8.3: "same key + same body returns the original
                # load_id". At Phase 1 we don't yet track per-key load_ids;
                # return 202 with a dedup-acknowledged status.
                return {
                    "status": "deduplicated",
                    "binding_id": binding_id,
                    "idempotency_key": x_idempotency_key,
