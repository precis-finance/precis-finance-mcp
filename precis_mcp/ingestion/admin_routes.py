...
"""Admin HTTP surface for the ingestion subsystem on the agui process.

HTTP surface for the ingestion admin UI. Routes mounted under
`/api/admin/`:

  - `POST /api/admin/reload_integrations` — refresh agui's
...
rest of the admin surface uses (`precis/routes/admin.py`). The JWT
middleware (`JWTAuthMiddleware` in agui) decodes the token and stamps
...
matching the pattern in `precis/routes/admin.py`.
"""
...
def build_admin_router(
    *,
    ...
    `registry_ref` — the live agui-side `IntegrationRegistryRef` constructed
    by `attach_ingestion_to_app`. Reads project from `.current`; writes
    rewrite the underlying YAML files and call `.reload()`.

    The router relies on `JWTAuthMiddleware` (mounted on the agui app) to
    ...
    async def reload_semantic_views(request: Request):
        ...
        try:
            report = apply_all(
                instance_root / "semantic", get_clickhouse_client()
            )
        except Exception as exc:
            _logger.exception(
    ...
    # Schemas — served from `precis/routes/admin.py` under
    ...
    async def trigger_load(body: dict, request: Request):
        if orchestrator_ctx is None:
            raise HTTPException(
                ...
                "PRECIS_ENABLE_PUSH_ENDPOINT=1 on the agui host to wire it.",
            )
...
def _audit(
    ...
    """Write a `security_audit_log` row — matches the audit pattern used
    by `precis/routes/admin.py`."""
    ...
    db.execute_platform(
        ...
        VALUES (%s, %s, %s::jsonb)
        """,
        (event_type, actor_id, json.dumps(payload)),
    )
