...
left untouched, because the commercial scenario-lifecycle tools
(`ScenarioStore`) mutate them at runtime (status, locks, new variants) and
...
# `semantic.scenarios` table created by CH migration 005. The insert always
...
def _scenarios_ddl(schema: str) -> str:
    ...
    Mirrors CH migration `005_semantic_scenarios.sql` — the schema is fixed;
    only the (qualified) name varies by `schema`.
    """
    return (
