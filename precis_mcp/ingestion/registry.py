...
CheckType = Literal[
    "not_null", "unique", "accepted_values", "accepted_range",
    "referential", "expression", "row_count", "distinct_count", "reconcile",
]

# A threshold compares a comparison operator against an integer failing-row
# count, e.g. "> 100", ">= 0", "!= 0". Absent → any failing row trips.
_THRESHOLD_RE = re.compile(r"^\s*(>=|<=|!=|=|>|<)\s*\d+\s*$")
# Referential target: 'schema.table.column'.
_REFERENCES_RE = re.compile(r"^[a-z_][a-z0-9_]*\.[a-z_][a-z0-9_]*\.[a-z_][a-z0-9_]*$")


class ReconcileMeasure(BaseModel):
    """One aggregate reconciled between staged and source, by name.

    `expr` is the staged-side aggregate (ClickHouse, over `staging.<x>`),
    e.g. ``"sum(amount)"`` / ``"count()"``. The same name must be a
    projected column of the check's ``source_query``. `tolerance` carries
    exactly one of ``abs`` (absolute gap) or ``pct`` (fractional gap).
    """

    model_config = ConfigDict(extra="forbid")

    expr: str
    tolerance: dict[Literal["abs", "pct"], float] = Field(
        default_factory=lambda: {"abs": 0.0}
    )

    @model_validator(mode="after")
    def _check(self) -> "ReconcileMeasure":
        if not self.expr.strip():
            raise ValueError("reconcile measure expr must be non-empty")
        if len(self.tolerance) != 1:
            raise ValueError(
                "reconcile measure tolerance must carry exactly one of "
                "{abs, pct}"
            )
        return self
...
class Check(BaseModel):
    """One data-quality control on a binding, run in the validate stage
    against ``staging.<x>`` before the swap.

    Two authoring forms, one severity grammar: a curated ``type`` (with
    its params) or a raw ``sql`` returning the offending rows. The
    failing-rows convention is uniform — **zero rows = pass**. ``severity``
    decides what a trip does: ``error`` blocks the swap; ``warning`` loads
    and records; ``info`` records only. ``threshold`` tolerates a failing
    count before the check trips at its severity.
    """
    ...
    # Exactly one authoring form.
    type: Optional[CheckType] = None
    sql: Optional[str] = None

    # Curated params (the subset each type consumes; see _validate_type_params).
    ...
    measures: Optional[dict[str, ReconcileMeasure]] = None
    ...
    def _check(self) -> "Check":
        ...
        if (self.type is None) == (self.sql is None):
            raise ValueError(
                f"check {self.name!r}: provide exactly one of 'type' or 'sql'"
            )
        if self.threshold is not None and not _THRESHOLD_RE.match(self.threshold):
            raise ValueError(
                f"check {self.name!r}: threshold={self.threshold!r} must be a "
                "comparison against an integer, e.g. '> 100'"
            )
        if self.type is not None:
            _validate_type_params(self)
        return self


def _validate_type_params(c: "Check") -> None:
    """Per-type required-param validation. `extra='forbid'` already
    rejects unknown keys; this enforces the params a given type needs."""
    def require(*fields: str) -> None:
        for f in fields:
            if getattr(c, f) in (None, [], {}):
                raise ValueError(
                    f"check {c.name!r} (type={c.type}): '{f}' is required"
                )
    ...
    if c.type == "not_null":
        ...
        require("columns")
    elif c.type == "accepted_values":
        require("column", "values")
    elif c.type == "accepted_range":
        if c.min is None and c.max is None:
            raise ValueError(
                f"check {c.name!r} (accepted_range): at least one of "
                "'min'/'max' is required"
            )
        require("column")
    elif c.type == "referential":
        require("column", "references")
        if not _REFERENCES_RE.match(c.references or ""):
            raise ValueError(
                f"check {c.name!r}: references={c.references!r} must be "
                "'schema.table.column'"
            )
    elif c.type == "expression":
        require("expression")
    elif c.type == "row_count":
        if c.min is None and c.max is None:
            raise ValueError(
                f"check {c.name!r} (row_count): at least one of 'min'/'max' "
                "is required"
            )
    elif c.type == "distinct_count":
        require("column")
        if c.min is None and c.max is None:
            raise ValueError(
                f"check {c.name!r} (distinct_count): at least one of "
                "'min'/'max' is required"
            )
    elif c.type == "reconcile":
        require("group_by", "measures", "source_query")
...
class Binding(BaseModel):
    ...
    def _check(self) -> "Binding":
        ...
        if dupes:
            raise ValueError(
                f"Binding {self.id!r}: duplicate check names {dupes} — "
                "each check name must be unique within a binding"
            )
        return self
