...
class RunRequest(BaseModel):
    ...
    a deliberate, audit-trailed reload of a closed period. Both are
    recorded on `load_history` (notes verbatim, override_lock as part of
    the `triggered_by` audit suffix); `override_lock` is audit-only and
    does not itself enforce period locking.
    """
    ...
    # matches the load_history CHECK. Required
...
class IdempotencyLoadIdCache:
    ...
    The cache stores `body_hash` so the endpoint can enforce the idempotency
...
def build_run_router(
    *,
    ...
    async def run(
        ...
        # -- Idempotency --------------------------------------------------
        # Same key + same body returns the original load_id;
