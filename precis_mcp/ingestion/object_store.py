...
def parse_host_key_entry(value: str) -> tuple[str, bytes]:
    """Parse a pinned host key — `<key-type> <base64>`, tolerating a full
    `ssh-keyscan` line (`<host> <key-type> <base64>`). Returns the key type
    and the decoded key blob."""
    import base64
    import binascii

    parts = value.split()
    if parts and not parts[0].startswith(("ssh-", "ecdsa-")):
        parts = parts[1:]
    if len(parts) < 2:
        raise ValueError(
            "Pinned SFTP host key must be '<key-type> <base64>' as printed "
            f"by ssh-keyscan; got {value!r}"
        )
    try:
        blob = base64.b64decode(parts[1], validate=True)
    except (binascii.Error, ValueError) as exc:
        raise ValueError(
            f"Pinned SFTP host key has invalid base64 key material: {value!r}"
        ) from exc
    return parts[0], blob
...
class SftpObjectStore:
    ...
    The server's host key is always verified — against a known_hosts file
    (`known_hosts_path`) or a single pinned key (`host_key`). Constructing
    the store with neither fails closed.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not known_hosts_path and not host_key:
            raise ValueError(
                "SFTP host-key verification is not configured. Set "
                "PRECIS_SFTP_KNOWN_HOSTS to a known_hosts file covering "
                f"{host!r} (generate with: ssh-keyscan -p {port} {host}), or "
                "PRECIS_SFTP_HOST_KEY to the server's public key "
                "('<key-type> <base64>'). Connecting without verifying the "
                "host key would let a man-in-the-middle capture the "
                "credentials and the data."
            )
        pinned = parse_host_key_entry(host_key) if host_key else None
        if known_hosts_path and not Path(known_hosts_path).is_file():
            raise FileNotFoundError(
                f"SFTP known_hosts file not found: {known_hosts_path}"
            )
        try:
            import paramiko  # type: ignore[import-not-found]
        except ImportError as exc:  # pragma: no cover
            raise ImportError(
                "SftpObjectStore requires paramiko; install the sftp extra "
                '(pip install ".[sftp]")'
            ) from exc
        ...
        if pinned:
            key_type, blob = pinned
            entry = host if port == 22 else f"[{host}]:{port}"
            client.get_host_keys().add(
                entry, key_type, paramiko.PKey.from_type_string(key_type, blob)
            )
        ...
        # Keep the SSHClient referenced — dropping it closes the transport.
        self._ssh = client
