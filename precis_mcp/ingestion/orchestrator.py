...
The driver protocol, dbt build, REPLACE-PARTITION-on-landing, and
landing-table DDL bootstrap are all gone post-refactor. The orchestrator
is just the glue holding the three new modules together — plus the
load_history, lock, span, and error-attribution skeleton that survived.
...
Each stage has its own status bucket on `load_history`:
  ...
  zero-row extract → `failed_validation` (refused before swap; an empty
                                         ...
                                         live partition)
  validate failure →  `failed_recon`    (reused legacy bucket; CHECK
                                         constraint accepts it without
                                         a schema migration)
