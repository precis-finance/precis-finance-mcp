...
the `reload_integrations` MCP tool, analogous to `CatalogueRef` in
`precis/server.py`.
"""
...
class IntegrationRegistryRef:
    ...
    Parallel to `CatalogueRef` in `precis/server.py`. Atomic swap on
...
def _pg_advisory_lock_factory(key: str, *, timeout: int, blocking_timeout: int):
    """Default ingestion lock — Postgres session-level advisory lock (ADR-0006).
...
def _build_ibis_backend(source: Source) -> Any:
    """Construct an Ibis backend for one Source from env-var credentials.

    Supported kinds:
      - `postgres` — `ibis.postgres.connect(...)`. Aggregation pushes
        down to PG; only result rows traverse the wire.
      - `http_upload` — `ibis.duckdb.connect()` (in-memory). DuckDB
        reads CSV / Parquet / JSON files natively via `read_csv` /
        `read_parquet`; the binding query references files relative to
        `${source_path}` (resolved at the orchestrator boundary).

    Credentials follow the same `<SECRET_REF_UPPER>_*` convention as
    `precis_mcp/engine/ibis_registry.py`. Kept here standalone so
    wiring doesn't depend on the engine module's singleton.
    """
    ...
    if source.kind == "postgres":
        ...
        port = int(os.environ.get(f"{prefix}_PORT") or 5432)
        missing = [
            s
            for s, v in (("HOST", host), ("USER", user), ("DATABASE", database))
            if not v
        ]
        if missing:
            raise IntegrationConfigError(
                f"Source {source.id!r} (kind=postgres) is missing required "
        ...
        # TLS is opt-in per source. A customer warehouse reached over a VPC or
        # the internet carries financial data + DWH credentials, so set
    ...
    if source.kind == "http_upload":
        # In-memory DuckDB. The binding's `extract.query` references
        # files via `read_csv('${source_path}/<file>.csv')`; the
        # orchestrator substitutes `${source_path}` with the resolved
        # absolute path before execution. No credentials needed —
        # LocalFs is unauthenticated.
        return ibis.duckdb.connect()

    raise IntegrationConfigError(
        f"Source {source.id!r} declares kind={source.kind!r} which is not "
        f"yet supported as an Ibis backend. Currently supported kinds: "
        f"postgres, http_upload. Add a branch to "
        f"wiring._build_ibis_backend and document in "
        f"docs/architecture/03-integration.md when extending."
    )
...
def _make_ibis_resolver(registry: IntegrationRegistry) -> Callable[[str], Any]:
    ...
    def resolve(source_id: str) -> Any:
        ...
        backend = _build_ibis_backend(source)
...
# ---------------------------------------------------------------------------
# Deployment-time attachment — called from server.py / agui.py at startup
# ---------------------------------------------------------------------------
...
def attach_ingestion_to_mcp(
    ...
    *,
    ...
    Called once from `precis/server.py` after the `FastMCP` instance
    is created. Returns the ref so other surfaces (admin routes, agent
    tools) can read from it.
    """
...
def attach_ingestion_to_app(
    ...
    *,
    ...
    `PRECIS_INGEST_UPLOAD_DIR` is set — keeps dev imports of `agui.py`
    from requiring privileged filesystem paths.
    """
...
# ---------------------------------------------------------------------------
# Object stores — kept for file-drop daemons. The orchestrator itself
# no longer uses them (Ibis owns warehouse credentials); file-drop
# ingestion is a separate code path with its own reshape pending.
# ---------------------------------------------------------------------------
...
def build_default_ingestion_context_from_env(
    *,
    ...
    (agui's push-endpoint attachment) treat None as "skip wiring, log
    only."
    """
...
def attach_admin_routes_to_app(
    ...
    *,
    ...
    `registry_ref` is the agui-side `IntegrationRegistryRef` — same one
