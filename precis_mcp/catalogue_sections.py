...
scenario registry (no agent runtime, no streaming), so both the commercial SPA
system prompt (via ``agent.skill_router``) and the open MCP ``instructions``
builder consume them from here.
"""
...
def render_scenarios_table(
    ...
    lines = [
        "## Available Scenarios",
        "",
        "Use the **scenario key** in tool calls. Real scenarios come from `semantic.scenarios`; shifted and comparison scenarios are generated.",
        "",
        "### Data scenarios (stored data)",
        "",
        "| Key | Scenario ID | Label | Status | Description |",
        "|---|---|---|---|---|",
    ]
    ...
    for row in vocabulary["real"]:
        lines.append(
            f"| `{row['key']}` | `{row['scenario_id']}` | {row['label']} | "
    ...
    lines.extend([
        ...
        "",
        "| Key | Base | Offset | Label | Description |",
        "|---|---|---|---|---|",
    ])
    for row in vocabulary["shifted"]:
        offset = f"{row['time_offset_months']:+d} months"
        lines.append(f"| `{row['key']}` | `{row['base']}` | {offset} | {row['label']} | {row['description']} |")
    ...
    for row in vocabulary["comparisons"]:
        ...
        lines.append(f"| `{row['key']}` | {formula} | {row['label']} | {row['description']} |")
