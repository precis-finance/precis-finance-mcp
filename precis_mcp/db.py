...
def get_redis_client():
    """Redis client for the commercial stack (plan-write locks, HITL preview
    cache). Lazy import: no open-core path uses Redis — the ingestion lock is
    Postgres-backed (ADR-0006) — so the open package does not depend on it."""
    import redis
...
# ---------------------------------------------------------------------------
# PostgreSQL — precis_platform (operational data: routines, dispatch, etc.)
#
...
def open_platform_pool() -> None:
    ...
    a connection. Mirrors the checkpointer's lifespan pattern
    (`precis/agent/checkpointer.py`).
    """
    _get_pool()
