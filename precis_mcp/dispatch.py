...
dependency on the agent runtime (LangChain/LangGraph) or on any commercial
tool module — those LLM-binding concerns live in the commercial
``agent.tool_registry`` thin layer, and the commercial tool *set* is injected
via :func:`register_tool_loader` (ADR-0008).

The open core ships only the read-only tool set (metric/statement/inspect,
list/search, ingestion reads, validation). The commercial product
...
- ``register_tool_loader()`` — commercial extension point: add a tool loader +
  its catalogue metadata (open core registers none)
...
TOOL_CATALOGUE: dict[str, dict[str, Any]] = {
    ...
    # These three previously had no catalogue entry and relied on the
    # ToolDescriptor defaults (base tool, access="read").  They now carry
    # explicit metadata so the catalogue is the full record, and the
    # `mcp_read` flag is what publishes them to the external transport.
...
# `register_tool_loader` from `precis.agent.commercial_tools` (ADR-0008).
...
# ---------------------------------------------------------------------------
# The open core ships only the read-only tool set. The commercial product
# registers its additional tools + catalogue metadata at startup via
# `register_tool_loader` (called from `agent.commercial_tools`). An open
# deployment registers none, so `build_descriptors` returns the open set and
# this module imports no commercial tool module (ADR-0008).
...
def register_tool_loader(
    ...
    """Register a commercial tool loader and its catalogue metadata.
...
def tool_catalogue() -> dict[str, dict[str, Any]]:
    """The full catalogue: open ``TOOL_CATALOGUE`` plus registered commercial
...
def _load_open_tools(reg: Any, catalogue_ref: Any) -> None:
    ...
    # locally; the agui process holds a separate ref for federated reads and
    # admin reload — see follow-up issue: those two refs do not share state,
    # so an admin reload via /api/admin/reload_integrations does not refresh
    # the agent's view of the registry. Acceptable today because the agent
    # tools only inspect static metadata (bindings, datasets); a reload that
    # the agent needs to see can wait for the next agui restart.
...
def _load_all_tools(catalogue_ref) -> dict[str, Any]:
    """Load the open tool set plus any registered commercial loaders."""
    reg = _ToolRegistry()
...
def build_descriptors(catalogue_ref) -> dict[str, ToolDescriptor]:
    ...
    LangChain ``StructuredTool`` (in the commercial ``agent.tool_registry``
    ...
    is the merged open + registered-commercial view (``tool_catalogue()``).
    """
...
def process_tool_call(
    ...
    # shape, reject anything else with a corrective error. See ISS-016.
...
def _audit_gate_denial(
    ...
    try:
        ...
        db.execute_platform(
            ...
            VALUES (%s, %s, %s::jsonb)
            """,
            ("tool_gate_denied", get_auth_context().user_id, json.dumps(details)),
        )
    except Exception:
        logger.exception(
...
def _audit_tool_outcome(
    ...
    try:
        ...
        db.execute_platform(
            ...
            VALUES (%s, %s, %s::jsonb)
            """,
            ("agent_tool_call", get_auth_context().user_id, json.dumps(details)),
        )
    except Exception:
        logger.exception(
