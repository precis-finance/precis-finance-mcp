...
so a reload is visible to both. Kept out of ``agui`` (commercial) so the open
MCP path can reach the catalogue without importing the agent application.
"""
...
def _load_initial_catalogue(catalogue_dir, loader=load_and_validate):
    ...
    that reads as a generic crash-loop (ISS-028). Re-raising preserves
...
class CatalogueRef(Protocol):
    ...
    modules don't reference the commercial server's concrete type."""
