...
class KeycloakConfig:
    ...
    specific (used only by the commercial SPA login); the open verification
...
def check_token_contract(*, fetch: bool = True) -> list[str]:
    ...
    if disc:
        if disc.get("jwks_uri") and disc["jwks_uri"] != jwks_url:
            problems.append(
                f"discovery jwks_uri ({disc['jwks_uri']}) != configured jwks_url "
                f"({jwks_url}) — set OIDC_JWKS_URL to match"
            )
