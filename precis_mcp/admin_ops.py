...
Open module shared by both admin surfaces: the Précis HTTP admin
...
# The Précis identity grain (user_profile_ext) is layered by the route via
...
def create_user(
    *,
    ...
    external IdP owns the credential, so no password is required. A failure
    after Keycloak provisioning rolls back the Keycloak account; the users
    insert commits per-statement, so a failure in ``on_db_insert`` leaves the
    row behind.
    """
    from precis_mcp.auth import ensure_user_directory
    from precis_mcp.db import execute_platform, query_platform, write_security_audit
    ...
    # Filesystem first: idempotent, and an empty directory left behind by a
    # later failure is harmless — unlike a committed users row or a deleted
    # Keycloak account.
    try:
        ensure_user_directory(id)
    except OSError as exc:
        raise ProvisioningError(
            f"cannot create user directory for '{id}': {exc}"
        ) from exc
    ...
    write_security_audit(
        "user_created", actor, target_user_id=id,
        details={
            "is_admin": is_admin,
...
def set_admin(*, user_id: str, is_admin: bool, actor: str) -> None:
    from precis_mcp.db import execute_platform, write_security_audit
...
def disable_user(*, user_id: str, actor: str, self_id: str | None = None) -> None:
    ...
    write_security_audit("user_disabled", actor, target_user_id=user_id)


def reset_password(*, user_id: str, password: str, actor: str) -> None:
    from precis_mcp.db import write_security_audit
    from precis_mcp.keycloak_admin import KeycloakAdmin, KeycloakAdminError
    ...
    try:
        ...
        raise ProvisioningError(str(exc)) from exc
    write_security_audit("password_reset", actor, target_user_id=user_id)
...
def assign_profile(*, user_id: str, profile_id: str, actor: str, source: str = "admin_ui") -> None:
    ...
    write_security_audit(
        "profile_assigned", actor, target_user_id=user_id,
...
def revoke_profile(*, user_id: str, actor: str) -> str:
    ...
    write_security_audit(
        "profile_revoked", actor, target_user_id=user_id,
        details={"profile_id": existing[0]["profile_id"]},
    )
    return existing[0]["profile_id"]


# --- open-grain reads (the CLI's list/show; the Précis route keeps its
...
def list_security_audit(
    *,
    ...
    """Read recent ``security_audit_log`` rows for operator review/export.

    Filters are ANDed; ``since`` is an ISO-8601 lower bound on ``created_at``.
    Newest first, capped by ``limit``. This is the open read surface for
    the audit log — the rows are also queryable directly in Postgres.
    """
    from precis_mcp.db import query_platform
    ...
    return [
        dict(r) for r in query_platform(
            "SELECT id, created_at, event_type, actor_id, target_user_id, "
            "scenario_id, trace_id, details FROM security_audit_log"
