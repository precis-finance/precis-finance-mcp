...
Headless operator surface for the platform-Postgres objects the open core owns:
...
shares all logic with the commercial admin UI via `precis_mcp.admin_ops`, so both
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser("reset-password", help="Reset a user's Keycloak password.")
    sp.add_argument("--id", required=True)
    sp.add_argument("--password", help="Omit to auto-generate a temporary password.")
