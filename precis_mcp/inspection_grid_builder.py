...
inspection-grid half of the open/commercial render boundary (ADR-0004) — the
...
1. ``streaming/blocks/inspection_grid.py`` — the SSE emitter for the chat
   transcript path (``inspect_rows`` with ``out='render'``).
2. ``mcp_external/server.py`` — the open MCP render seam.

The returned dict is already in the shape the frontend ``InspectionGrid``
renderer expects.
"""
