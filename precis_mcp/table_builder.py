...
for scale labels). It is the finance-table half of the open/commercial render
boundary (ADR-0004): the open MCP transport and the SSE/report paths all build
the same block from here.

Consumed by:

1. ``streaming/blocks/financial_table.py`` — the SSE emitter for the chat
   transcript path (``run_metric`` / ``run_statement`` with ``out='render'``).
2. ``tools/report_sink.py`` — the report block sink for the
   ``run_metric(out='report')`` / ``run_statement(out='report')`` path.
3. ``excel/block_emit.py`` — the Excel statement emitter.

Input is the unified result schema (see the "Result schema" section of
``docs/engine_totals_rollup_plan.md``): top-level ``kind`` / ``dimensions`` /
...
The returned dict is already in the shape the frontend ``FinancialTable``
