...
-- (from planning.entries) plus four statistical-account sections
...
WITH unified AS (
    ...
    -- Plan / Budget / Forecast scenarios
    SELECT
        'ENT-001'                   AS entity_id,
        ...
        CASE WHEN e.account IN ('9200', '9210') THEN 'avg'
             ELSE 'sum'
    ...
    GROUP BY e.account, e.cost_centre, e.period, e.scenario,
             COALESCE(e.commit_id, '__uncommitted__')
